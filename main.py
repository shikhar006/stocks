		...
		if b == True:
			...
			for i in stocks:
				...
				def __high__():
					...
					selling_percent = (selling_dollars / stocks[stock_symbol][0]) * 100
...
def f_sell():
	...
	def __sell__():
		...
		if stock_symbol in stocks:
			...
			result = requests.get(f"https://money.cnn.com/quote/quote.html?symb={stock_symbol}").text
			soup = BeautifulSoup(result, features="lxml")
			stock_price_current = soup.find("span",attrs = {"streamformat": "ToHundredth"})
			stock_price_current = str(stock_price_current)
			stock_price_current = stock_price_current.split("<")
			stock_price_current = stock_price_current[1]
			stock_price_current = stock_price_current.split(">")
			...
			def __Enter__():
				...
				if quantity > stock_quantity:
					messagebox.showinfo("Error", "Quantity higher than you have")
					ent_quantity.delete(0, END)
				else:
					price = stock_price_current * quantity
					price = str(price)

					label_price = Label(root_sell, text = "The selling price is "+ price + "$", width = 30)
